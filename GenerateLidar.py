...
# color values used for segmentation images
seg_colors = {
  0: (0, 0, 0),
  1: (70, 70, 70),
  2: (100, 40, 40),
...
def detect_static_objects(tag_img, tag_value, threshold):
    thresholded_image = cv2.inRange(tag_img, tag_value, tag_value)
...
def draw_and_save_bounding_boxes(img, img_labeled, img_tag, filename):
  bboxes = []
  ...
  # detect traffic lights and signs, and add to bounding box array
  tl_bbox_array = detect_static_objects(img_tag, 18, 10)
  ts_bbox_array = detect_static_objects(img_tag, 12, 10)
  for bbox in tl_bbox_array:
    bboxes.append(bbox)
    filtered_classes.append(5)
  for bbox in ts_bbox_array:
    bboxes.append(bbox)
    filtered_classes.append(6)
...
def main():
  ...
  with os.scandir(path) as dir:
    for pcl_file in dir:
      ...
      img_range, img_intensity, img_seg, img_labeled, img_tag = processPointCloud(path + filename + '.ply')
      img_bbox = draw_and_save_bounding_boxes(img_range, img_labeled, img_tag, filename)
