...
def main():
    ...
    try:
        ...
        for car in car_list:
            car_blueprints.append(world.get_blueprint_library().filter('vehicle.' + car)[0])
        for truck in truck_list:
            truck_blueprints.append(world.get_blueprint_library().filter('vehicle.' + truck)[0])
        for bicycle in bicycle_list:
            bicycle_blueprints.append(world.get_blueprint_library().filter('vehicle.' + bicycle)[0])
        ...
        for n, transform in enumerate(spawn_points):
            if n >= args.number_of_vehicles:
                break
            if (n >= 0 and n < 20):
                blueprint_choice = car_blueprints
            if (n >= 20 and n < 40):
                blueprint_choice = truck_blueprints
            if (n >= 40 and n < 60):
                blueprint_choice = bicycle_blueprints
            blueprint = random.choice(blueprint_choice)
