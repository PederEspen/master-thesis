...
from collectLidarData import saveActorLabels

from carla_vehicle_annotator import get_bounding_box, get_vehicle_class
...
if not os.path.exists(dir_seg):
    os.makedirs(dir_seg)
if not os.path.exists(dir_abbox):
...
class PedestrianBoundingBoxes(object):
    ...
    def draw_bounding_boxes(display, bounding_boxes):
        ...
        if pedestrian_bbox_record == True:
            f = open("PedestrianBBox/bbox"+str(count) + '.txt', 'w')
            print("PedestrianBoundingBox")
        for bbox in bounding_boxes:
            points = [4, [(int(bbox[i, 0]), int(bbox[i, 1])) for i in range(8)]]
...
class VehicleBoundingBoxes(object):
    ...
    @staticmethod
    def get_bounding_boxes(vehicles, pedestrians, camera):
        """
        ...
        for vehicle in vehicles:
            bounding_boxes2.append([vehicle.id, vehicle.type_id, VehicleBoundingBoxes.get_bounding_box(vehicle, camera)])

        for pedestrian in pedestrians:
            bounding_boxes2.append([pedestrian.id, pedestrian.type_id, PedestrianBoundingBoxes.get_bounding_box(pedestrian, camera)])
    ...
    def game_loop(self, args):
        ...
        try:
            ...
            while True:
                ...
                bounding_boxes = VehicleBoundingBoxes.get_bounding_boxes(vehicles, pedestrians, self.camera)
